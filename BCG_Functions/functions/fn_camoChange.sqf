...
if (_currentUniform == "") then {
	...
	if (_uniformCamoTypes find _camoType != -1) then {
		...
		_msg = format ["WARNING: Camo type <%1> is not a viable camo under the 'camoTypes' config field of the current uniform, <%2>. The uniform was not changed.",_camoType,_currentUniform];
		RPT_DTAIL(_msg,__FILE__,__LINE__);
	};
};
